...
?>
    ...
    if ($id) {
        ...
        pedido.idproductoFinal, productoFinal.nombre as productof, pedido.idempleado, concat(empleado.nomb, ' ',empleado.apaterno) as empleado, pedido.idestado, 
        estado.estado as estado from pedido inner join productoFinal on productoFinal.idproductoFinal = pedido.idproductoFinal inner join
        empleado on empleado.idempleado = pedido.idempleado inner join estado on estado.idestado = pedido.idestado 
        where pedido.serie = ? ;";
    ...
    <div class="bloque">
        ...
        <form method="POST" name="frmpedido" id="frmpedido" action="../Controlador/controladorPedido.php?accion=actualizar"
            ...
            <select name="idestado" id="idestado">
                <?php
                // Verifica que la variable $estados esté disponible

                foreach ($estados as $estado): ?>
                    <option value="<?php echo htmlspecialchars($estado['idestado'], ENT_QUOTES, 'UTF-8'); ?>"
